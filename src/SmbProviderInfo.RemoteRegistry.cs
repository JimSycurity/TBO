...
{
	public sealed class RemoteRegistrySession : IAsyncDisposable, IDisposable
	{
		internal RemoteRegistrySession(RemoteRegistryClient client, Smb2TreeConnect share, Stream stream)
		{
		...
		public RemoteRegistryClient Client { get; }
	...
	{
		internal async Task<RemoteRegistrySession> OpenRemoteRegistrySessionAsync(
			string serverName,
			CancellationToken cancellationToken)
		{
			...
			var client = new RemoteRegistryClient();
			Smb2TreeConnect? share = null;
			Smb2Pipe? pipe = null;
			Stream? stream = null;
			var pipeName = client.WellKnownPipeName ?? "winreg";
			...
			try
			{
				...
				pipe = await OpenRegistryPipeAsync(share, pipeName, cancellationToken).ConfigureAwait(false);
		...
		private async Task<Smb2Pipe> OpenRegistryPipeAsync(
